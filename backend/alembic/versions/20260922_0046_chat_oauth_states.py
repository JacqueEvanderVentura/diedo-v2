...
def upgrade() -> None:
    ...
    if "chat_oauth_states" not in existing_tables:
        op.create_table(
            "chat_oauth_states",
    ...
    inspector = sa.inspect(bind)
    existing_indexes = {index["name"] for index in inspector.get_indexes("chat_oauth_states")}
    if "ix_chat_oauth_states_workspace_expires" not in existing_indexes:
        op.create_index(
