...
export function ActivoFormModal({ open, onClose, activo }) {
  ...
  useEffect(() => {
    if (open) {
      setForm(activo ? { ...activo, code: activo.code || '', value: activo.value ?? '' } : EMPTY)
      setErr('')
    }
  }, [open, activo])
  ...
  const submit = () => {
    ...
    if (editing) {
      updateActivo(activo.id, form)
      toast.success(`"${form.name}" actualizado`)
    } else {
      addActivo(form)
      toast.success(`"${form.name}" registrado`)
    }
    onClose()
  }
  ...
  return (
    <Modal open={open} onClose={onClose} title={editing ? 'Editar activo' : 'Nuevo activo'} testId="activo-form-modal">
      <div className="space-y-4">
        ...
        {err && <p className="text-sm font-medium text-red-500" data-testid="activo-form-error">{err}</p>}

        <div className="flex gap-3 pt-1">
          <Button variant="secondary" className="flex-1" onClick={onClose} data-testid="activo-form-cancel">Cancelar</Button>
          <Button className="flex-1" onClick={submit} data-testid="activo-form-save">{editing ? 'Guardar cambios' : 'Registrar activo'}</Button>
        </div>
      </div>
    </Modal>
  )
