...
| P1 | Descubrimiento CRM (`VITE_FEATURE_CRM_DISCOVERY`) | Integración backend implementada con SerpAPI principal, Serper de respaldo y cuota por workspace; falta validación con credenciales reales en preproducción. | Configurar secretos staging, validar proveedores reales, revisar términos/cuotas y agregar observabilidad operativa. | Búsqueda auditada, limitada y sin claves en el navegador, con recepción de resultados reales en staging. |
...
| P1 | Chat (`VITE_FEATURE_CHAT`) | Fases 1–5: esquema, webhooks, REST inbox, OAuth + asignación de sucursales en Configuración. | UI inbox (`/chat`), webhook público live y App Review (fases 4 y 6). | Mensajes IG/WA por sucursal con cuentas compartibles; flag off en prod hasta go-live. |
